...
export interface WalletTransactionDto {
  walletId: number;
  type: 'DEPOSIT' | 'WITHDRAW' | 'TRANSFER_IN' | 'TRANSFER_OUT' | 'PURCHASE' | 'SALE' | 'REWARD' | 'PENALTY';
  amount: number;
  description?: string;
  referenceId?: string;
  metadata?: object;
...
export class WalletService {
  ...
  async getOrCreateWallet(dto: CreateWalletDto): Promise<Wallet> {
    ...
    if (dto.serverId) {
      ...
      if (!server) {
        throw new NotFoundException('Server not found or does not belong to the specified game');
      }
    }
  ...
  // 특정 게임의 지갑들 조회
  async getUserWalletsByGame(userId: number, gameId: number): Promise<Wallet[]> {
    return await this.walletRepository.find({
  ...
  // 지갑 거래 (트랜잭션)
  async executeTransaction(dto: WalletTransactionDto): Promise<WalletTransaction> {
  ...
  // 지갑 잔액 조회
  async getWalletBalance(walletId: number): Promise<{ balance: number; locked_balance: number; available_balance: number }> {
