...
export class CreateGamesAndServersTable20250629000100 implements MigrationInterface {
    ...
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'name', type: 'varchar', length: '100' },
          { name: 'description', type: 'varchar', length: '500', isNullable: true },
          { name: 'server_url', type: 'varchar', length: '255', isNullable: true },
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (gameServersTable) {
      const foreignKey = gameServersTable.foreignKeys.find(fk => fk.columnNames.indexOf('game_id') !== -1);
      if (foreignKey) {
