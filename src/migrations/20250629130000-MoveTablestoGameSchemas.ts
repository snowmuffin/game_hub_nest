...
export class MoveTablestoGameSchemas1751198300000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const storageItemsExists = await queryRunner.hasTable(
    ...
    if (storageItemsExists) {
      ...
      `);
      console.log(
        '✅ Moved online_storage_items -> space_engineers.online_storage_items',
      );
    }
    ...
    if (marketplaceExists) {
      ...
      `);
      console.log(
        '✅ Moved marketplace_items -> space_engineers.marketplace_items',
      );
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const tableName of seTables) {
      ...
      if (tableExists) {
        ...
        `);
        console.log(
          `✅ Restored space_engineers.${tableName} -> public.${tableName}`,
        );
      }
    }
    ...
    for (const tableName of valheimTables) {
      ...
      if (tableExists) {
        ...
        `);
        console.log(
          `✅ Restored valheim.${tableName} -> public.${publicTableName}`,
        );
      }
    }
