...
export class CreateWalletsAndCurrenciesTables20250329000100
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (walletsTable) {
      const userForeignKey = walletsTable.foreignKeys.find(
        (fk) => fk.columnNames.indexOf('user_id') !== -1,
      );
      const currencyForeignKey = walletsTable.foreignKeys.find(
