...
import { Repository, LessThanOrEqual, Like, QueryRunner } from 'typeorm';
...
export class DropTableService {
  ...
  async update(id: number, updateDropTableDto: Partial<DropTable>): Promise<DropTable> {
  ...
  // 게임에서 사용할 드롭 계산 메서드
  async calculateGameDrop(damage: number, mult: number, maxRarity: number): Promise<string | null> {
    ...
    for (const item of dropItems) {
      const adjustedWeight = Math.pow(mult, item.rarity) * item.drop_rate_multiplier;
  ...
  async getDropTableItems(options: {
    ...
    const queryBuilder = this.dropTableRepository.createQueryBuilder('drop_table');
  ...
  async getAvailableItemsFromItemsTable(options: {
    ...
    if (options.rarity !== undefined) {
      queryBuilder.andWhere('items.rarity = :rarity', { rarity: options.rarity });
    }
    ...
    queryBuilder.orderBy('items.rarity', 'ASC').addOrderBy('items.displayName', 'ASC');
  ...
  async addDropTableItem(createDropTableDto: {
    ...
    if (!itemExists) {
      throw new BadRequestException(`Item with ID ${createDropTableDto.item_id} does not exist in items table`);
    }
    ...
    if (existingDropItem) {
      throw new BadRequestException(`Item with ID ${createDropTableDto.item_id} already exists in drop table`);
    }
    ...
    const dropTableItem = this.dropTableRepository.create({
      ...
      is_active: createDropTableDto.is_active !== undefined ? createDropTableDto.is_active : true,
  ...
  async updateDropTableItem(id: number, updateDropTableDto: {
    drop_rate_multiplier?: number;
    is_active?: boolean;
    description?: string;
  }): Promise<DropTable> {
  ...
  async getDropTableStats(): Promise<{
    ...
    const activeItems = await this.dropTableRepository.count({ where: { is_active: true } });
